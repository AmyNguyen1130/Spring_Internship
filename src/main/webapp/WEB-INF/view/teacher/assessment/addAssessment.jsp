		...
		<div class="row = 12">
			<div class="col-sm-8">
				...
				<form class="form-group"
					action="<%=request.getContextPath()%>/teacher/assessment/saveAddAssessment"
					method="post">
					Assessment Name: <input class="form-control" type="text" name="assessmentname">
					<p class="error">${error.errAssessmentName}</p>
					Class: <select class="form-control" id="classid" name="classid">
						<option value="">None</option>
						<c:forEach items="${listClass}" var="class">
							<option value="${class.classid}">${class.classname}</option>
						</c:forEach>
					</select> <br> Start Date: <input class="form-control" type="date" id="startdate" name="startdate">
					...
					<br> <input type="hidden" value="true" name="status" />
