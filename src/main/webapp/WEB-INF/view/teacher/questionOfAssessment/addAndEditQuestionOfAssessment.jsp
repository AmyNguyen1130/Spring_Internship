				...
				<div class="col-sm-4">
					<c:forEach begin="0" end="3" step="1" varStatus="num">
						...
						<div class="optionItem">
							...
							<input type="text" class="form-control" name="options[${num.index}].optionValue" value="${questionInf.getOptions().size() > num.index ? questionInf.getOptions().get(num.index).getOptionValue() : ''}">
							<input type="radio" name="correctanswer" value="${name}" class="radioOption" ${questionInf.getCorrectanswer() == name ? 'checked' : '' } ${num.index == 0 ? 'checked' : ''}>
						</div>
					</c:forEach>
				</div>
			</div>
